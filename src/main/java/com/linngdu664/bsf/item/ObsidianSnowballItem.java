...
public class ObsidianSnowballItem extends Item {
    public ObsidianSnowballItem() {
        ...
        DispenserBlock.registerBehavior(this, new AbstractProjectileDispenseBehavior() {
            protected Projectile getProjectile(Level p_123476_, Position p_123477_, ItemStack p_123478_) {
                return Util.make(new AdvancedSnowballEntity(p_123476_, p_123477_.x(), p_123477_.y(), p_123477_.z(), SnowballType.OBSIDIAN, 6.0F, 8.0F), (p_123474_) -> {
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        ...
        if (pPlayer.getOffhandItem().getItem() == ItemRegister.EMPTY_SNOWBALL_STORAGE_TANK.get()) {
            pPlayer.setItemInHand(InteractionHand.OFF_HAND, new ItemStack(ItemRegister.OBSIDIAN_SNOWBALL_STORAGE_TANK.get()));
        ...
        } else if (pPlayer.getOffhandItem().getItem() == ItemRegister.OBSIDIAN_SNOWBALL_STORAGE_TANK.get() && pPlayer.getOffhandItem().getDamageValue() != 0) {
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        pTooltipComponents.add(new TranslatableComponent("obsidian_snowball.tooltip").withStyle(ChatFormatting.GRAY));
    }
}
