    ...
    public @NotNull InteractionResultHolder<ItemStack> use(@NotNull Level pLevel, Player pPlayer, @NotNull InteractionHand pUsedHand) {
        ...
        if (!storageInTank(pPlayer, itemStack, ItemRegister.EXPLOSIVE_SNOWBALL_STORAGE_TANK.get())) {
            ...
            if (!pLevel.isClientSide) {
                ExplosiveSnowballEntity snowballEntity = new ExplosiveSnowballEntity(pPlayer, pLevel, getLaunchFunc(getSnowballReDamageRate(pPlayer)));
        ...
        if (pPlayer.getOffhandItem().getItem() == ItemRegister.EMPTY_SNOWBALL_STORAGE_TANK.get()) {
            ...
            if (!pLevel.isClientSide) {
                ...
                ExplosiveSnowballEntity snowballEntity = new ExplosiveSnowballEntity(pPlayer, pLevel, getLaunchFunc(1));
