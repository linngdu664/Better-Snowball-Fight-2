...
public class StoneSnowballItem extends BSFSnowballItem {
    ...
    public @NotNull InteractionResultHolder<ItemStack> use(@NotNull Level pLevel, Player pPlayer, @NotNull InteractionHand pUsedHand) {
        ...
        if (!storageInTank(pPlayer, itemStack, ItemRegister.STONE_SNOWBALL_STORAGE_TANK.get())) {
            pLevel.playSound(null, pPlayer.getX(), pPlayer.getY(), pPlayer.getZ(), SoundEvents.SNOWBALL_THROW, SoundSource.NEUTRAL, 0.5F, 0.4F / (pLevel.getRandom().nextFloat() * 0.4F + 0.8F));
            if (!pLevel.isClientSide) {
                StoneSnowballEntity snowballEntity = new StoneSnowballEntity(pPlayer, pLevel, getLaunchFunc(getSnowballReDamageRate(pPlayer)));
        ...
        if (pPlayer.getOffhandItem().getItem() == ItemRegister.EMPTY_SNOWBALL_STORAGE_TANK.get()) {
            pPlayer.setItemInHand(InteractionHand.OFF_HAND, new ItemStack(ItemRegister.STONE_SNOWBALL_STORAGE_TANK.get()));
        ...
        } else if (pPlayer.getOffhandItem().getItem() == ItemRegister.STONE_SNOWBALL_STORAGE_TANK.get() && pPlayer.getOffhandItem().getDamageValue() != 0) {
            ...
            if (!pLevel.isClientSide) {
                ...
                StoneSnowballEntity snowballEntity = new StoneSnowballEntity(pPlayer, pLevel, getLaunchFunc(j));
    ...
    public void appendHoverText(@NotNull ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, @NotNull TooltipFlag pIsAdvanced) {
        ...
        pTooltipComponents.add(new TranslatableComponent("stone_snowball.tooltip").withStyle(ChatFormatting.BLUE));
    }
}
